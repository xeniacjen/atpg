...
void Atpg::TransitionDelayFaultATPG( FaultList& faultListToGen, PatternProcessor* pcoll, int& untest )
{
	...
	GENERATION_STATUS result = patternGeneration(Fault(fTDF.gate_+cir_->ngate_,fTDF.type_,fTDF.line_,fTDF.aggr_));
...
{
	GENERATION_STATUS result = patternGeneration(*faultListToGen.front());
	if( result == TEST_FOUND ){
		Pattern *p = new Pattern;
		p->pi1_ = new Value[cir_->npi_];
		p->ppi_ = new Value[cir_->nppi_];
...
//              in:    fault
//              out:   GENERATION_STATUS(TEST_FOUND = 0, UNTESTABLE, ABORT)
...
// Date       [ KOREAL Ver. 1.0 started 2013/08/10 ]
// **************************************************************************
Atpg::GENERATION_STATUS Atpg::patternGeneration(Fault fault) {
	...
	//SET A FAULT SIGNAL
    pFaultyLine = initialize(fault,BackImpLevel,implyStatus);
...
Gate* Atpg::initialize(Fault& fault,int& BackImpLevel,IMP_STATUS& implyStatus){
    currentFault_ = fault;
...
void Atpg::initialNetlist(Gate& gFaultyLine )
{
    for (int i = 0 ; i < cir_->tgate_ ; i ++){
        Gate &g = cir_->gates_[i];
...
bool Atpg::xPathTracing(Gate* pGate)
{
    ...
    for (i = 0 ; i < pGate->nfo_ ; i ++){
        Gate *pFanoutGate = &cir_->gates_[ pGate->fos_[i] ];

        if (xPathTracing(pFanoutGate)){
            xPathStatus_[pGate->id_] = X_EXIST;
            return true;
        }
    }
...
// Commentor  [ CKY ]
// Synopsis   [ usage: help to get n0 n1 and Value depend on Gate type
